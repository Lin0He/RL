...
def Ablation(env):
    ...
    env = gym.make("CartPole-v1")
    DQL = DQLAgent(env, 3, 64, Adam(learning_rate=0.001))
    TotalRewards = Q_learn(env, DQL, episodes, gamma=.99, replay=True,  replaySize = 128, targetNetwork=False,  n_update = 10,  policy='egreedy',
            epsilon=0.3, ablation=True)
    TotalRewards = Tool.smooth(TotalRewards, 101)
    DQL.ResetWeights()
    ...
    TotalRewards = Q_learn(env, DQL, episodes, gamma=.99, replay=False,  replaySize = 128, targetNetwork=True,  n_update = 10,  policy='egreedy',
            epsilon=0.3, ablation=True)
    DQL.ResetWeights()
    TotalRewards = Tool.smooth(TotalRewards, 101)
    ...
    env = gym.make("CartPole-v1")
    DQL = DQLAgent(env, 3, 64, Adam(learning_rate=0.001))
    TotalRewards = Q_learn(env, DQL, episodes, gamma=.99, replay=True,  replaySize = 128, targetNetwork=True,  n_update = 10,  policy='egreedy',
            epsilon=0.3, ablation=True)
    DQL.ResetWeights()
    TotalRewards = Tool.smooth(TotalRewards, 101)
